...
- [ ] **fix-careers-hero-watermark** - Remove the "Fabricio" photo-credit watermark from the `/careers` hero (light grey text next to the "come join us" headline, verified at native resolution). Scope widened by the second opinion: fix the source asset `public/images/yum-chef-team.jpg` itself (repaint or replace), because `lib/site.ts` also uses that file as the careers OG/social-share image, so a page-level recrop alone would still ship the watermark into link previews. Undercuts the site's otherwise-consistent "real people, not stock" photography.
- [ ] **fix-accessibility-statement-content** - Write an actual accessibility statement on `/accessibility-statement` (conformance standard, what's been done, how to report an issue, response time). The page currently has no statement content at all under that title, only a feedback form. Confirmed by both passes.
...
- [ ] **dedupe-patticake-repeated-blocks** - Scope doubled by the second opinion: four blocks repeat near-verbatim across the Patticake page trio, not two. "send cake, not a card" and "the restaurant behind patticake" appear on `/`, `/patticake`, and `/order-a-cake`; the full "choose how the cake travels" section and the identical "make it sound like them" message-maker section repeat across `/patticake` and `/order-a-cake`; the ticker is shared too. Pick one canonical home per block (recommend: home page for the two brand blocks) and either drop the repeats or give them genuinely page-specific photo/copy.
- [ ] **consolidate-patticake-process-explainers** - Four different, overlapping step-by-step "how it works" frameworks across five placements describe what is really one send-a-cake process: home's 3-step "find the sweetest route", "choose how the cake travels" (which runs on both `/patticake` and `/order-a-cake`), `/patticake`'s 4-step "a clearer way to send it", and `/order-a-cake`'s 4-step "from message to table, clearly". Consolidate into one framework (same step count, same wording) reused consistently. Coordinate with dedupe-patticake-repeated-blocks, since "choose how the cake travels" appears in both tasks.
...
- [ ] **fix-careers-form-layout** - Promoted from P2 by the second opinion: the `/careers` application form layout is visibly broken, with roughly fifteen fields crammed into the right half of the form card while the left half sits dead empty for ~1,300 vertical pixels, before the essay question jumps columns. It reads as an unstyled ATS embed. Fix the layout, cut the "Highest Degree Achieved?" question (out of place for hourly restaurant work), and add an open-roles/hiring-locations block above the long form.
- [ ] **fix-reviews-star-rounding** - The reviews walls on `/patticake` and `/yum-kitchen` render five fully-filled stars beside "4.5", while the location page correctly renders 4.4 as four filled plus one gray, so the component can do partial ratings but rounds 4.5 up to a visually dishonest five-star row on the two highest-traffic walls. Render 4.5 as four and a half stars. While in there: `/patticake`'s "loved across the twin cities" headline is backed exclusively by St. Louis Park ratings; consider more honest framing.
...
- [ ] **dedupe-in-the-news-archive** - The curated and archive sections list the same stories twice under reworded titles ("good company founder interview" / "yum! on good company", both KSTP 2023; "woodbury opening feature" / "yum! in woodbury magazine", both Woodbury Magazine 2023): dedupe them. Also make clear whether the category filter pills apply to the 22-item archive grid below the curated section. Per the second opinion, no heavy visual de-emphasis is needed; the archive layout itself is tidy and every item is attributed.
- [ ] **fix-contact-cta-redundancy** - Resolve `/contact`'s 3-way call/form/call-again redundancy (a visitor can finish filling the form before learning calling is faster), and add a path for catering and press inquiries, both of which the rest of the site clearly cares about.
- [ ] **tailor-thank-you-confirmation** - Give `/thank-you` a response-time line and, where feasible, tailor the message (or at least the suggested next step) to which form was actually submitted; "Browse Menu" right after a feedback or accessibility complaint is a non sequitur.
...
- [ ] **fix-kitchen-banner-promise** - "see what's coming out of the kitchen right now" promises fresh content but only links out to Instagram/TikTok. It ships sitewide via `components/ReviewsWall.tsx` (Yum home, `/patticake`, and every location page), not just the location template, so populate it with real content or soften the promise as a single shared-component change.
