...
- B6: PARTIAL / blocked on GitHub Actions minutes. Code side done 2026-07-09: `verify.yml` now triggers on push + PR to main (PR #10), and a `SKIP_LH` guard skips the hardware-sensitive Lighthouse step in CI so the gate is deterministic (PR #11; `SKIP_LH=1 bash verify.sh` -> VERIFY PASSED locally). Blocker: the "Verify" workflow was `disabled_manually` and, once re-enabled, dispatched runs sit **queued with no runner** - the signature of exhausted GitHub-hosted Actions minutes / a spending-limit hold on this private repo (last successful runs were 2026-07-01). Left the workflow re-parked as `disabled_manually` to avoid stuck-pending checks on every PR. To finish: Zach checks GitHub -> Settings -> Billing -> Actions (restore minutes / raise spending limit), then re-enable the workflow (Actions tab -> "Enable workflow", or `gh api -X PUT repos/zachringnight/yumkitchen-rebuild/actions/workflows/304973316/enable`). No further code change needed.
